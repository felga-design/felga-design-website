...
export default function KoloryComp({ data, title }: Props) {
  ...
  // Stwórz pełną listę obrazków, zachowując kolejność
  ...
  // Licznik przesunięcia indexu dla openGallery()
  let globalIndex = 0;
...
//   return (
//     <section className="inset-0 bg-gradient-to-br from-[#1e252a] via-[#0c1015] to-[#dbc87d1a] text-white">
//       <section className="max-w-screen-2xl mx-auto px-6 md:px-0 flex gap-8 pt-16 pb-40">
//         {/* === ZAWARTOŚĆ WEDŁUG KATEGORII === */}
...
//                     (item) =>
//                       item.title === realizacja.title &&
//                       item.description === realizacja.description &&
//                       item.image.src === realizacja.image.src
//                   );

//                   return (
//                     <div
//                       key={`${realizacja.title}-${index}`}
//                       onClick={() => openGallery(index)}
//                       className="cursor-pointer group relative w-full mx-auto h-[350px] md:w-[380px] rounded-[4px]  border-[#cfd6e20e] bg-gradient-to-tr from-[#252A30] from-45% to-[#181C1F] overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-[#171b1f] hover:scale-101 hover:-translate-y-1 transition-all duration-300"
//                     >
//                       <div className="relative h-[270px] w-full">
//                         <Image
//                           src={realizacja.image.src}
...
//         {/* === STICKY MENU === */}
//         <nav className="hidden md:block sticky top-20 h-max bg-[#19191adb] w-[330px] rounded-md shadow-lg p-6">
//           <h2 className="text-2xl font-bold mb-4">{title}</h2>
//           <ul className="space-y-2 text-sm md:text-base">
//             {Object.entries(grouped).map(([slug, group]) => (
//               <li key={slug}>
...
//           <div
//             onClick={(e) => e.stopPropagation()}
//             className="relative w-full max-w-6xl h-[80vh] bg-black rounded-lg"
//           >
//             <Gallery images={images} initialIndex={initialIndex} />
//             <button
//               onClick={closeGallery}
